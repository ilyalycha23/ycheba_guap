...
class Logger:
    ...
    def _iter_temperature_docs(self) -> List[Dict[str, Any]]:
        if self._mongo_db is not None:
            return list(self._mongo_db["Temperature"].find())
        return list(self._mem_temp)

    def average_temperature(self) -> Optional[float]:
        """Лаб. 8: среднее значение температуры."""
        docs = self._iter_temperature_docs()
        vals = [d.get("Temperature") for d in docs if d.get("Temperature") is not None]
        if not vals:
            return None
        return sum(vals) / len(vals)

    def max_temperature(self) -> Optional[float]:
        """Лаб. 8: вторая характеристика — максимум."""
        docs = self._iter_temperature_docs()
        vals = [d.get("Temperature") for d in docs if d.get("Temperature") is not None]
        if not vals:
            return None
    ...
    def chart_series(self, limit: int = 30) -> Dict[str, List[Any]]:
        """Лаб. 9: метки времени и значения для Chart.js."""
        docs = self._iter_temperature_docs()[-limit:]
        labels = [d.get("timeStamp", "") for d in docs]
        values = [d.get("Temperature", 0) for d in docs]
